...
public class SimpleBankingApp {
	...
	public static void loadUserData() {
		// structure of each record: username (email address), password, first_name,
		// last_name, mobile_number

		// in the ideal case (real deployment of the app), we will read from file or
		// database, but let's hard-code for now
	...
	public static void loadAccountData() {
		...
		// account number, username (email) of account holder, account type (Standard or
		// Saving), account_opening_date

		// in the ideal case, we will read from file or database, but let's hard-code
		// for now
		Account anAccount;
		try {
	...
	public static void printAllAccounts() {
		...
		// System.out.println("Account_number | username_of_account_holder |
		// account_type | account_opening_date");
	...
	public static void addTransaction(String account_number, double amount) {
		...
		// If the account exists, add the transaction
		if (accountExists) {
			...
			System.out.println("ERROR: Account number does not exist: " + account_number);
		}
	}

	public static double getBalance(String account_number) {
		// Initialize a variable to hold the balance and set its initial value to 0
		double balance = 0;

		// Iterate through each transaction in the transactions vector
		for (Transaction transaction : transactions) {
			// Check if the transaction's account number matches the given account number
			if (transaction.getAccount_number().equals(account_number)) {
				// If they match, add the transaction amount to the balance
	...
	public static void main(String[] args) {
		...
		addTransaction("9999-1111", 21.00); // it seems this account does not exist in the loaded (populated) data,
											// but the addTransaction does not do that check, need to improve that
											// function in future
		// let's print the accounts and their balance to see if the above transaction
		// have impacted their balances
