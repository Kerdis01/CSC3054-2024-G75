...
public class SimpleBankingAppTest {
	...
	// this test method (test case) verifies if the data load feature of the class
	// (unit or component)
	// under test (UUT) works properly
	public static void testLoadData() {
		// Reminder: the classical Four-Phase test pattern
		...
		// 3-Verify phase
		// we see in the load function of the UUT that we have loaded 3 users, so let's
		// verify that
		if (SimpleBankingApp.users.size() == 3)
			System.out.println(
					TestUtils.TEXT_COLOR_GREEN + "testLoadData: loadUserData: TC1 passed" + TestUtils.TEXT_COLOR_RESET);
		else
			System.out.println(
					TestUtils.TEXT_COLOR_RED + "testLoadData: loadUserData: TC1 FAILED" + TestUtils.TEXT_COLOR_RESET);

		// The above only verification is basic (simple, weak)
		// To do STRONGER verification, we would need more assertions for user names and
		// account balances, etc.

		SimpleBankingApp.loadAccountData();
		if (SimpleBankingApp.accounts.size() == 4)
			System.out.println(TestUtils.TEXT_COLOR_GREEN + "testLoadData: loadAccountData: TC1 passed"
					+ TestUtils.TEXT_COLOR_RESET);
		else
			System.out.println(TestUtils.TEXT_COLOR_RED + "testLoadData: loadAccountData: TC1 FAILED"
					+ TestUtils.TEXT_COLOR_RESET);

		// 4-Teardown phase: if our goal was to only test the load, as Teardown
		// (mainApp.accounts)
		// we would have deleted the loaded deleted from memory (variables users, and
		// accounts), but we want
		// to use those data in the other tests, thus, we do not do any Teardown in this
		// test case
	}
	...
	public static void testDeposits() {
		...
		// read more about the tear-down phase of test cases:
		// http://xunitpatterns.com/Four%20Phase%20Test.html
		SimpleBankingApp.addTransaction("5495-1234", -depositAmount);
	}
	...
	public static void testWithdrawals() {
		// 1-Setup phase
		double balanceBefore = SimpleBankingApp.getBalance("5495-1234");
		double withdrawalAmount = 30.00; // Predefined withdrawal amount

		// 2-Exercise phase
		SimpleBankingApp.addTransaction("5495-1234", -withdrawalAmount); // Notice the negative sign for withdrawal
		double balanceAfter = SimpleBankingApp.getBalance("5495-1234");
		...
		if (balanceBefore - withdrawalAmount == balanceAfter)
			...
			System.out.format(
					"testWithdrawals: balanceBefore = %.2f ; withdrawalAmount = %.2f ; balanceAfter = %.2f %s\n",
					balanceBefore, withdrawalAmount, balanceAfter, TestUtils.TEXT_COLOR_RESET);
		}

		// 4-tear-down
		// To reverse the withdrawal, add back the amount withdrawn
